...
const performOCR = async (req, res) => {
  ...
  // Input validation: check base64 is present and is string
  ...
  // Validate base64 size - limit to 50MB
...
const getOwnerBookings = async (req, res, next) => {
  try {
    ...
    // Optimize N+1: Use aggregation pipeline instead of nested finds
    const bookings = await Booking.aggregate([
      ...
      {
        $match: {
          "spot.owner": new (require("mongoose")).Types.ObjectId(ownerId),
        },
      },
      {
        $lookup: {
