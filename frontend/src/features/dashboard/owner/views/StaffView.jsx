...
export default function StaffView() {
  ...
  const performOCR = async (doc) => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || "OCR failed - click to retry", true);
    } finally {
      setOcrLoading((prev) => {
        const updated = { ...prev };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        const errMsg = apiErr.response?.data?.message || apiErr.message || "Operation failed";
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8 space-y-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          {/* ── Live status indicator — icon only ── */}
          <div className="w-9 h-9 flex items-center justify-center bg-white border border-slate-100 shadow-sm rounded-full">
            {refreshing ? (
              <i className="fas fa-spinner animate-spin text-indigo-400 text-sm" />
            ) : initialLoading ? (
              <i className="fas fa-circle-notch animate-spin text-slate-300 text-sm" />
            ) : (
          ...
          >
            <i
              className={`fas fa-rotate-right text-slate-400 text-sm ${refreshing ? "animate-spin" : ""}`}
            />
          </button>
          ...
          <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[95vh] overflow-hidden flex flex-col">
            ...
            >
              ...
              <div>
                ...
                >
                  <i className="fas fa-cloud-upload-alt text-3xl text-slate-300 mb-2" />
