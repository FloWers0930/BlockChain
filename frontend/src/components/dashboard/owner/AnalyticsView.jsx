...
export default function AnalyticsView() {
  ...
  const fetchAnalytics = useCallback(async (isRefresh = false) => {
    // Show refreshing indicator for live/manual updates (keep old data visible)
    if (isRefresh) {
      setRefreshing(true);
    } else {
      setError(null);
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8 space-y-7">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <StatCard
          ...
          icon={<i className="fas fa-charging-station" />}
          accent="#3b82f6"
          loading={initialLoading}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative">
          ...
          <p className="text-slate-400 text-xs mb-6">
            Latest transactions across all stations
          </p>
          ...
          {initialLoading ? (
            ...
            <div className="h-48 flex flex-col items-center justify-center">
              <i className="fas fa-receipt text-4xl text-slate-200 mb-3" />
              <p className="text-sm text-slate-400">No recent bookings</p>
            </div>
          ) : (
            <div className="space-y-1 max-h-[380px] overflow-y-auto pr-1 -mr-1">
              {recentBookings.map((b, i) => (
                <div
                  key={b._id ?? i}
                  className="flex justify-between items-center px-3 py-3 rounded-2xl
                             hover:bg-slate-50 transition-colors duration-150 group"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0">
                      <i className="fas fa-car text-slate-400 text-xs" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 truncate">
                        {b.spot?.location || "Unknown Station"}
                      </p>
